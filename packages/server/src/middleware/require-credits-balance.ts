...
const MINIMUM_CREDIT_THRESHOLD = 15;
...
export const requireCreditsBalance = createMiddleware<AuthenticatedEnv>(
  async (c, next) => {
    try {
      ...
      if (creditsBalance <= MINIMUM_CREDIT_THRESHOLD) {
        return c.json(
          {
            error: `Low credit balance: ${creditsBalance} credits left. Run /upgrade to buy more credits.`,
          },
          402,
        );
      }
